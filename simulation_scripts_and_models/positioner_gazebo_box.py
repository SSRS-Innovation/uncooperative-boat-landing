# Behöver göra om hela denna till en funtion kanske??? funktion utan input borde typ funka
def positioner():
    import cv2 
    ...
    # Drone connection
    drone_connection = mavutil.mavlink_connection('udpin:localhost:14552')
    boat_connection = mavutil.mavlink_connection('udpin:localhost:14562') 
    ...
    gps_counter = 0 # to give gps data to autopilot
    ...
    def on_new_boxes(msg: AnnotatedAxisAligned2DBox_V): # boxes message 
        ...
        if msg.annotated_box:
            ...
            # print(bool(msg.annotated_box and msg.annotated_box[0].label == 8 and img_size[0]*0.8 > bbox_center > img_size[0]*0.2))
            if msg.annotated_box[0].label == 8 and img_size[0]*0.8 > bbox_center[0] > img_size[0]*0.2:
    ...
    print("positioner Waiting for heartbeat from drone")
    drone_connection.wait_heartbeat()
    print("positioner waiting for heartbeat from boat drone")
    boat_connection.wait_heartbeat()
    print("positioner Heartbeat from system (system %u component %u)" % (drone_connection.target_system, drone_connection.target_component))
    ...
    try:
        while True:
            ...
            AHRS_boat = boat_connection.recv_match(type = 'AHRS2', blocking = False)
            ...
            gps_counter += 1
            # print(AHRS_boat)
            # the fake AIS data uncomment/comment to enable/disable
            if AHRS_boat is not None and gps_counter >= 50: 
                gps_boat_lat, gps_boat_long = AHRS_boat.lat/1e7, AHRS_boat.lng/1e7 # boats position?
                yk = np.array([gps_boat_lat, gps_boat_long, 0,0]).transpose()
                
                xk, P = kalman.kalman_filter(yk, xk, P, A, Q, H, R)
                drone_connection.mav.command_int_send(drone_connection.target_system, drone_connection.target_component, 0, mavutil.mavlink.MAV_CMD_DO_SET_HOME, 0, 0, 0, 0, 0, 0, int(xk[0,0]*1e7), int(xk[1,1]*1e7), 0) # kalman filtered OBS ALT 0
