...
def boat_angledim_compensator(boat_dims, boat_pic_angle, boat_bearing, drone_bearing, rel_bearing): #, boat_distance, bbox_centerangle):
    ...
    # v = drone_bearingdeg - boat_bearing + boat_pic_angle # mod 360 fixes - negative degrees to postiive as we want ti
...
def boat_distance(effective_len, camera_matrix, bbox):
    ...
    # compen_len = boat_angledim_compensator(boat_dims, boat_bearing, drone_bearing)  # this function is not working right now 
    compen_len = 4.2 # effective_len #
    ...
    dist = focal_l*compen_len/(bbox.xyxy[0][2]-bbox.xyxy[0][0])  # blir en tensor här pga bbox kanske dåligt?
    return dist
